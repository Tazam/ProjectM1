...
public class QuoteAnnotatorCustom implements Annotator{
	 ...
	 */
	   ...
	   *                 attributes.
	   */
	  public QuoteAnnotatorCustom(String name, Properties props) {
	   ...
	   *                 attributes.
	   */
	  public QuoteAnnotatorCustom(Properties props) {
	   ...
	   */
	  public QuoteAnnotatorCustom(String name, Properties props, boolean verbose) {
	  ...
	  public Pair<List<Pair<Integer, Integer>>, List<Pair<Integer, Integer>>>  recursiveQuotes(String text, int offset, String prevQuote) {
	    ...
	    if (!isAQuoteMapStarter(start, quotesMap) && start >= 0 && start < text.length() - 3) {
	      ...
	      for (Pair<Integer, Integer> e : embedded.first()) {
	        quotes.add(new Pair<>(e.first() + start + quote.length(),
	            e.second() + start + 1));
            System.out.println("quoteadd start " + e.first().toString() + start + quote.length()+ ", " + e.second().toString() + start + 1);
	    ...
	    for (String qKind : quotesMap.keySet()) {
	      for (Pair<Integer, Integer> q : quotesMap.get(qKind)) {
	        if (q.second() - q.first() >= qKind.length() * 2) {
	          ...
	          for (Pair<Integer, Integer> e : embedded.first()) {
	            ...
	            if (e.second() - e.first() > 2) {
	              ...
	              System.out.println("quoteadd e " + e.first().toString() + ", " + e.second().toString());
	        ...
	        System.out.println("quoteadd " + q.first().toString() + offset + ", " + q.second().toString() + offset);
	      }
	    }
